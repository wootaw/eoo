defmodule Eoo.CSV do
  ...
  defstruct [
    :filename, :options, :default_sheet,
    cells: %{}, cell_types: %{}, cells_read: false,
    first_row: nil, last_row: nil,
    first_column: nil, last_column: nil,
    sheets: ["default"]
  ]

  def open(filename, options \\ []) do
    unless File.exists?(filename), do: raise(Eoo.FileNotFound, message: "file #{filename} does not exist")
    csv_opts = Keyword.get(options, :csv_options, [])
    separator = Keyword.get(csv_opts, :separator, ",")
    csv = %__MODULE__{filename: filename, options: options}
  ...
  def default_sheet(csv, sheet) when is_binary(sheet) do
    if sheet in csv.sheets, do: {:ok, %{csv | default_sheet: sheet}}, else: {:error, "sheet '#{sheet}' not found"}
  end
  ...
  def row(csv, row_number, _sheet \\ nil) do
    f = csv.first_column || 1; l = csv.last_column || 1
  ...
  def column(csv, col_number, _sheet) when is_integer(col_number) do
    f = csv.first_row || 1; l = csv.last_row || 1
  ...
  def set(csv, row, col, value, _sheet \\ nil) do
    ...
    {:ok, %{csv | cells: Map.put(csv.cells, {row, col}, value), cell_types: Map.put(csv.cell_types, {row, col}, ct)}}
  end
  ...
  defp read_all_cells(csv, separator) do
    ...
    %{csv | cells: cells, cell_types: types, cells_read: true,
      first_row: (if row_count > 0, do: 1), last_row: (if row_count > 0, do: row_count),
      first_column: (if max_col > 0, do: 1), last_column: (if max_col > 0, do: max_col)}
  end
  ...
  defp parse_csv_fields([?\" | rest], sep, acc, cur, false), do: parse_csv_fields(rest, sep, acc, cur, true)
